...
app.get('/states', (req, res) => {
    let n
    if (req.query.state) {
        n = nigeriaState.States.filter((state) => req.query.state.toLowerCase() === state.state.toLowerCase())
    }
    res.send(n || nigeriaState)
})
...
// serving give away aspa airtime = mypromise()
...
app.get('/bio', (req, res) => {
    ...
    res.send({
        profile: {
            ...
            age: 25,
            likes: ['Football', 'running', 'coding'],
            occupation: 'Back-End Developer (NodeJs)',
            about: 'I dont have time to check time, it is what it is. so in time, there is no time!',
            target: 'intercept DARK WEB and littley heal the World',
            Besties: ['General BN Nwankwo', 'Nora', '(JSON)', 'Ella', 'Collins', 'Ernest', 'Madukaife', 'NwankwoJr', 'Lato-Babe', 'Naria-BamBam', 'OBO', 'Chika']      
        }
    })
})
